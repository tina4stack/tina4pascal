...
| barcode-scanner | 🟡 core+contract · ❌ shells | **Tina4 custom** — a live camera scanner. The core lays out a placeholder box and surfaces it through the embed pipeline as **kind 2** (`tina4_embed_*`), capturing the `formats` attr (e.g. `"qr,ean13"`) and boolean `torch`; a decode reported by a shell via `TinaScanResult` fires the element's `onscan` action with the exact decoded value (by-name dispatch — the value is never parsed as call syntax). **Camera preview + barcode decoding are per-shell and not yet wired:** AVFoundation built-in metadata (iOS/macOS), CameraX+MLKit/ZXing (Android), MediaCapture/V4L2 + decoder (desktop). Covered by `tests/test_scanner.pas` |
