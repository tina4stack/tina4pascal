...
macOS Cocoa `AVPlayerView` 🟡 loop TODO), `audio` (engine-drawn `ckAudio`
control + `AudioPlay`/`AudioProgress` contract, not a native overlay),
`canvas` (pure-Pascal `Tina4Canvas2D`),
