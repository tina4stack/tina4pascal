...
| background-blend-mode | 🟡 | blends a **gradient** background with the background-color beneath it — the separable modes (multiply/screen/overlay/darken/lighten/color-dodge/-burn/hard-/soft-light/difference/exclusion) **and** the non-separable `saturation`/`luminosity` are computed per-pixel through the shared `BlendRGB` (verified 0.00% vs Chrome). `hue`/`color` are correct in the shared code (raster path pixel-matches Chrome) but skew on the macOS `DrawRGBA` blit for a multi-stop gradient — a Cocoa-only residual. `url()` image layers not yet. (Note: macOS `--snapshot` now emits **sRGB**, not the display's P3, so saturated colours compare byte-exact to headless Chrome) |
