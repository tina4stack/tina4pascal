...
| direction | ✅ | `ltr`/`rtl`/`auto`, from the property, the `dir` attribute, or `dir="auto"` (first-strong detection). Mixed LTR/RTL lines are reordered logical→visual by the Unicode Bidi Algorithm L2 rule (Hebrew/Arabic classification, base level, simplified neutral resolution), verified pixel-matching Chrome. Native text backends shape each run. Remaining: per-character UBA (mixed direction inside one word), explicit embeddings/overrides (`bdi`/`bdo`/`unicode-bidi`), mirrored punctuation, RTL on the raster path. Reftest `bidi-rtl-ltr` |
